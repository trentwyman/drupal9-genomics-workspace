(function() {
    ...
    ;(function(factory) {
        ...
        var compare = function(ev,jQueryel,s,cfg) {
            // compare mouse positions to see if pointer has slowed enough to trigger `over` function
            if ( Math.sqrt( (s.pX-cX)*(s.pX-cX) + (s.pY-cY)*(s.pY-cY) ) < cfg.sensitivity ) {
                jQueryel.off(s.event,track);
                delete s.timeoutId;
                ...
                return cfg.over.apply(jQueryel[0],[ev]);
            } else {
                // set previous coordinates for next comparison
                s.pX = cX; s.pY = cY;
                // use self-calling timeout, guarantees intervals are spaced out properly (avoids JavaScript timer bugs)
                s.timeoutId = setTimeout( function(){compare(ev, jQueryel, s, cfg);} , cfg.interval );
            }
        };
    
        // triggers given `out` function at configured `timeout` after a mouseleave and clears state
        var delay = function(ev,jQueryel,s,out) {
            delete jQueryel.data('hoverIntent')[s.id];
            return out.apply(jQueryel[0],[ev]);
        };
        ...
        jQuery.fn.hoverIntent = function(handlerIn,handlerOut,selector) {
            ...
            var handleHover = function(e) {
                ...
                var hoverIntentData = jQueryel.data('hoverIntent');
                if (!hoverIntentData) { jQueryel.data('hoverIntent', (hoverIntentData = {})); }
                ...
                if (e.type === 'mouseenter') {
                    ...
                    jQueryel.off(mousemove,track).on(mousemove,track);
                    // start polling interval (self-calling timeout) to compare mouse coordinates over time
                    state.timeoutId = setTimeout( function(){compare(ev,jQueryel,state,cfg);} , cfg.interval );
                } else { // "mouseleave"
                    // do nothing if not already active
                    if (!state.isActive) { return; }
                    // unbind expensive mousemove event
                    jQueryel.off(mousemove,track);
                    // if hoverIntent state is true, then call the mouseOut function after the specified delay
                    state.timeoutId = setTimeout( function(){delay(ev,jQueryel,state,cfg.out);} , cfg.timeout );
                }
            };
        ...
        showLabel: function( element, message ) {
          ...
          if ( error.length ) {
            ...
            if ( error.is( "label" ) ) {
              ...
              errorID = error.attr( "id" ).replace( /(:|\.|\[|\]|\jQuery)/g, "\\jQuery1");
